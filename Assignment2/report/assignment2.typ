...
While somewhat beyond the scope of this report, we may considerjustify an equation for the forward model by referring to the derivation of the _eikonal equation_, @eq:eikonal, from applied to Maxwell's equations @born-optics[3.1.1].


In particular, this assumes no


---

from first principles by referring to the derivation from Maxwell's equations in @born-optics[3.1.1] considering the curls of the electric and magnetic fields through Maxwell's equations:
$
  ∇ × vv(E) &= - pdv(vv(B), t)\
  ∇ × vv(B) &= μ_0 (vv(J) + ε_0 pdv(vv(E), t))
$

which results in a tomographic inversion problem in which the observed delay in the travel times of the electromagnetic radar wave between the source and its corresponding network of receivers is used to reconstruct the spatial distribution of propagation
...
// The optimal value of $alpha^2$, as colored green, gives a smoother and more physically plausible decay of the power spectrum, reflecting a compromise between resolution and stability.
...
// The residual distribution for the optimal $L_2$-regularized solution is shown in @fig:l2-residual. It is strongly peaked around zero and approximately symmetric, indicating that the inversion captures most of the low frequency components, but it is clearly not normally distributed. However, the residuals exhibit heavier tails than a Gaussian distribution and are more closely matched by a Laplace distribution. This suggests that the errors are not purely Gaussian and may include modelling errors, outliers, or unresolved physical processes. This behavior highlights a limitation of the non-robust $L_2$ formulation and motivates the use of robust $L_1$ regularization.
])
})
