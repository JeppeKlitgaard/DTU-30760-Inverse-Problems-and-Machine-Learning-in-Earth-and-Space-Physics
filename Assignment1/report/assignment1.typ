...
We wish to be able to compute estimates of the covariance and resolution matrices of our model, which will give insight into respectively the uncertainty and bias of our obtained model, $vvh(m)$. Additionally, we may propogate the model covariance onto the prediction to produce estimates of the uncertainty of the obtained radial magnetic field.
...
Notably, since neither of the iteratively constructed weight matrices feature, this regularisation can be carried out without resorting to @alg:irls-r.
...
The non-robust $L_2$-regularized solution is obtained by minimizing a quadratic objective function under the assumption that the residuals are Gaussian distributed. To select an appropriate regularization parameter, the L-curve method is applied. @fig:l2-curve shows the L-curve as a scatter plot, where the corner identifies the optimal trade-off between data misfit and model norm. The corresponding regularization parameter is $alpha^2 = 1.60 times 10^(-8).$
...
#figure(
  ...
  caption: [
    Large version of the plot shown in @fig:l2-field-map
  ]
)
