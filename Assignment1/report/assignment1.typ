...
#let fig-l2-different_power_spectra = [
  #box(width: 50%)[
    #figure(
      ...
      caption: [power spectra for different L2 regularization strengths]
...
This value provides a reasonable balance between fitting the observations and suppressing unstable regions with high spatial frequencies model parameters.
...
The residual distribution for the optimal $L_2$-regularized solution is shown in @fig:l2-residual. It is strongly peaked around zero and approximately symmetric, indicating that the inversion captures most of the large-scale signal, but it is clearly not normally distributed. However, the residuals exhibit heavier tails than a Gaussian distribution and are more closely matched by a Laplace distribution. This suggests that the errors are not purely Gaussian and may include modelling errors, outliers, or unresolved physical processes. This behavior highlights a limitation of the non-robust $L_2$ formulation and motivates the use of robust $L_1$ regularization.
