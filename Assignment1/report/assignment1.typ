...
We wish to be able to compute the covariance and resolution matrices of our model, which will give insight into respectively the uncertainty and bias of our obtained model, $vvh(m)$. TODO: Additionally prediction covariance.
...
This value gives a reasonable balance between the data misfit and the model norm. 
...
$
As the $alpha^2$ gets smaller, the regularization becomes weaker and the inversion starts to fit short-wavelength 
...
#meander.reflow({
    import meander: *
  placed(top + right, stack(fig-l2-curve, v(1em), fig-l2-field-map, v(1em), fig-l2-power-spectrum, v(1em), fig-l2-residual))
  container()
  content([
structure more strongly.
As the $alpha^2$ gets larger, the regularization becomes stronger and overly smooth, meanwhile, the small-field is suppressed.
...
TODO: Laplacian distribution and Huber weights, discussion

#pagebreak(weak: true)
== Robust $L_1$-Regularised Solution <sec:results-l1>
#let alpha_sq_l1_star = num("1.6E-4")
...
#let fig-covariance-l1 = [
  #box(width: 50%)[
    #figure(
      ...
      caption: [
        The model covariance matrix and its associated standard deviations. Jagged dependence on index arises from increments in degree of spherical harmonics.
      ]
    ) <fig:covariance-l1>
  ]
]
...
If we inspect the power spectrum of the solution shown in @fig:spectrum-l1, we find that the contribution to the total power spectrum decreases for higher-order components of the model — this is encouraging, as a diverging power spectrum would indicate that the truncated orders would be significant and the model derived in @sec:theory-physics would need to be reconsidered. The fact that most of the power is contained in the first degree can be understood through the largely dipolar shape of the obtained field, where the field lines mostly leave the core in the southern hemisphere and enter in the northern hemisphere. A perfectly dipole would, of course, be captured wholly by the first degree, as evident by @eq:semi-discrete-model.

It is also worthwhile to consider the rate of decay in the components of @fig:spectrum-l1, which has largely leveled off at a high constant value of $≈ qty("4e9", "nT^2")$ at higher degrees. This is somewhat concerning as it implies that the truncated degrees may contribute significantly to the  prediction of the field at the CMB. We will return to this point in the discussion. TODO: Return in discussion
])
})


=== Covariance and Resolution Characteristics
#meander.reflow({
  import meander: *
...
When also considering higher degrees, we recover a region of flux reversed relative to the simple dipole model. In order to give credence to its existence, we first compute an estimate of  model covariance matrix, $mm(Σ)_vvh(m)$ by the method described in @sec:math-cov-res-matrix. We note again that the estimate is produced under a number of crude assumptions about the data, in particular that the data errors are uncorrelated and have a common variance.

The square of the first 50 entries of the model data covariance are shown in @fig:covariance-l1 in the diagonal can be seen to dominate until approximately the 14th degree. This is at least in part a result of having used the _plug-in_ estimate for the data covariance $Σ_vvh(d)$ as forewarned. In the lower figure the ordering of the Gauss coefficients becomes apparent by jagged nature of the parameter standard deviations. The flattened parameter index $i$ follows the natural ordering of @eq:semi-discrete-model and each saw-tooth of the plot is attributed an increment of the degree $n$ the associated orders $0≤m≤n$ immediately following it. Thus, higher indices generally belong to higher frequency components of the model.
])
})
...
TODO: Discuss why this is not reasonable (Magneto iono sphere)
...
It may be initially surprising that the standard deviation decreases for higher-frequency Gauss coefficients, but this arises from the bias introduced by the regularisation. As the spherical degree $n$ increases, the signal at the satellite becomes increasingly attentuated by the $(a\/r)^(n+2)$ factor of @eq:semi-discrete-model since $a\/r_"sat" ≈ 0.925$. This is what leads to instability in the unregularised problem as discussed in @sec:math-illposedness, but in the regularised problem these terms are instead dominated by the regularisation scheme, which in this case penalises the $L_1$ norm of the projection onto the CMB. This drives the high-frequency coefficients towards zero and explains the phenomenon observed in @fig:covariance-l1.

To give credence to this hypothesis, we compute the model resolution matrix, $mm(R)_vvh(m)$, which is depicted in @fig:resolution-l1. It largely follows the same shape as @fig:covariance-l1, but the interpretation is different — where the covariance matrix contains information about the variance of the model parameters, the resolution matrix offers insights into the _bias_ of the model. This can be understood by its construction from the pseudo-inverse and forward projection, which probes the null-space of the inversion. Diagonal entries close to unity correspond with perfectly resolved parameters, whereas values close to zero indicate that only very little information about those parts of the model are recovered.

We observe that the latter is the case for the high-frequency components, which are almost entirely lost during the inversion as a result of the strong attenuation discussed in @sec:math-illposedness. This also suggests that even if higher degrees were included in the model, we would not be able to derive significant information from them — the higher degrees are already dominated by the bias introduced through the regulariser. 

In conclusion, we find that the model has poor resolution of smaller spatial features and reconstruction of those are dominated by the a priori choice of regulariser.

Lastly, we may compute estimates of the variance in the magnetic field predictions at the CMB, which are the diagonal entries of $Σ_vvh(d)_"CMB"$ described in @sec:math-cov-res-matrix. 
...
#subpar.grid(
  ...
  figure(image("export/pred_std_dev_L1.png"), caption: [
    Estimated standard deviations of predicted field at CMB
...
TODO: NOTE THAT EDGES (HIGH FREQUENCIES) BETWEEN REGIONS ARE WHAT CANNOT BE RESOLVED, AGREES WITH ABOVE.

TODO Discussion: High frequency parts of L2 and L1 models are both determined mostly by choice of norm, compare these in discussion
])
})
...
assumption improvement, data 
model, n iteration, 
N=20 in discussion the power spectrum
